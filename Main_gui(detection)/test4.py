...
def identify_product(class_counts, product_dict):
    for product_name, specs in product_dict.items():
        normalized_specs = {k.lower(): int(v) for k, v in specs.items() if k.lower() != 'barcode'}
        if normalized_specs == class_counts:
            return product_name
    return None
